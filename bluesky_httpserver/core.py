...
#         raise UnsupportedMediaTypes(
#             "This type is supported in general but there was an error packing this specific"
#             f" data: {err.args[0]}",
#         )
...
class MsgpackResponse(Response):
    ...
    def render(self, content: Any, _reentered=False) -> bytes:
        try:
            with record_timing(self.__metrics, "pack"):
                return msgpack.packb(content, default=_fallback_msgpack_encoder, datetime=True)
            ...
            # it is not called.
            if "can not serialize 'datetime.datetime' object" in str(err) and (not _reentered):
...
def json_or_msgpack(request, content, expires=None, headers=None):
    ...
    return NumpySafeJSONResponse(content, headers=headers, metrics=request.state.metrics)

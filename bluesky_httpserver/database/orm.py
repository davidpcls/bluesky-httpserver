...
class Timestamped:
    ...
    time_updated = Column(DateTime(timezone=False), onupdate=func.now())  # null until first update

    def __repr__(self):
        return (
            f"{type(self).__name__}("
            + ", ".join(f"{key}={value!r}" for key, value in self.__dict__.items() if not key.startswith("_"))
            + ")"
        )
...
class APIKey(Timestamped, Base):
    ...
    hashed_secret = Column(LargeBinary(32), primary_key=True, index=True, nullable=False)
